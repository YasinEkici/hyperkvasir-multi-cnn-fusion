...
\end{itemize}
Sınıflandırıcı tüm bu karşılaştırmalarda sabittir ve yalnızca bir çok katmanlı
algılayıcı (MLP) olarak seçilmiştir; bu nedenle sınıflandırıcı bazlı bir
karşılaştırma yapılmamış, karşılaştırmalar yalnızca yukarıdaki üç eksende
yürütülmüştür.
...
ablasyonların hiçbiri, \%95 güven aralığı düzeyinde çapraz-doğrulamadaki en iyi
çapraz-entropi yapılandırmasını (üçlü ağırlıklı fine-tune) istatistiksel olarak
geçememiştir; nihai model olarak bu yapılandırmanın TTA'lı sürümü seçilmiştir.
Sayısal sonuçlar ve değerlendirme Bölüm~\ref{sec:results} ve
Bölüm~\ref{sec:discussion}'de verilir.
