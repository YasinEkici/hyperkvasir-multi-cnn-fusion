...
% Mimari diyagramı henüz çizilmedi (elle hazırlanacak). Çizildiğinde
% reports/final/figures/architecture.pdf olarak eklenip aşağıdaki figür
% ortamı yorumdan çıkarılacaktır.
\TODO{Mimari diyagramı (R/M/E omurga $\rightarrow$ 512 projeksiyon $\rightarrow$
füzyon $\rightarrow$ MLP) Şekil olarak eklenecek --- elle çizilecek.}
...
aktivasyonundan oluşur (PLD-04, PLD-05). Bu adım hem boyut uyumsuzluğunu giderir
...
güvenileceğinin eğitim sırasında ayarlanabilmesidir. (Kaynak:
\texttt{src/models/fusion/weighted.py}.)
...
genelleştirir. (Kaynak: \texttt{src/models/fusion/gmu.py}.)
...
dropout içerir. Proje kararları gereği sınıflandırıcı tüm deneylerde MLP olarak
sabit tutulmuştur (PLD-06); SVM, RandomForest veya XGBoost gibi alternatif
sınıflandırıcılar değerlendirilmemiştir. Bu sayede karşılaştırmalar omurga ağ,
...
loss~\cite{lin2017focal} denenmiştir (karar D-06). Lin ve ark.\ §3.2, eşitlik
...
$p_t$'ye dayanan focal modülasyonuyla çakışır (D-06). Bu nedenle focal
yapılandırması (\texttt{finetune\_wide\_focal.yaml}), exp 11'in CE
yapılandırmasından yalnızca kayıp bloğunda ayrılır; diğer tüm hiperparametreler
aynıdır (temiz, tek değişkenli karşılaştırma).

\subsection{Test Zamanı Veri Artırma (TTA)}
\label{subsec:tta}
Donmuş final modelin çıkarım reçetesi, eğitim maliyeti olmadan uygulanan
...
(karar D-07). Seed topluluğu, aynı yapılandırmanın farklı tohumlarla (seed)
...
ortalanır; argmax oylaması kullanılmaz. \textbf{Katlar arası saf model ortalaması
yasaktır}: 5 kat modelini tüm veri kümesi üzerinde ortalamak sızıntıya yol açar,
